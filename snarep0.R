...
ggsave(filename=paste0(outdir, "/p0_supp2.pdf"), 
...
nfy_plot <- ggplot(region_embedding, aes(x=UMAP1, y=UMAP2, color=NFY.NFY))+geom_point(size=ps)+
...
tbx_plot <- ggplot(region_embedding, aes(x=UMAP1, y=UMAP2, color=TBX.2.TBX))+geom_point(size=ps)+
  theme_Nice() + theme(legend.position="right") + labs(color="TBX/2")
hox_plot <- ggplot(region_embedding, aes(x=UMAP1, y=UMAP2, color=HD.14.homeodomain))+geom_point(size=ps)+
