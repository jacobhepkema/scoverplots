...
read_meme <- function(meme_path){
  require(stringr)
  meme_lines <- readLines(meme_path)
  MOTIF_start_idc <- str_detect(meme_lines, "^MOTIF")
  motif_length <- as.numeric(str_split(meme_lines[which(MOTIF_start_idc)[1]+1], " ")[[1]][6])
  motif_names <- str_split_fixed(meme_lines[MOTIF_start_idc], " ", n=2)[,2]
  all_motifs <- list()
  i <- 1
  for(start_i in which(MOTIF_start_idc)){
    curr_ppm <- t(apply(str_split_fixed(meme_lines[(start_i+2):(start_i+2+motif_length-1)], 
                                      "\t", n=4), 
                      2, as.numeric))
    rownames(curr_ppm) <- c("A", "C", "G", "T")
    all_motifs[[motif_names[i]]] <- curr_ppm
    i <- i + 1
  }
  return(all_motifs)
}
